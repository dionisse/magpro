...
const STATUS_MAP: Record<OrderStatus, { label: string; cls: string }> = {
  pending:    { label: 'En attente',    cls: 'bg-odoo-warning/15 text-odoo-warning' },
  processing: { label: 'En traitement', cls: 'bg-odoo-info/15 text-odoo-info' },
  shipped:    { label: 'Expédiée',      cls: 'bg-odoo-primary/15 text-odoo-primary' },
  delivered:  { label: 'Livrée',        cls: 'bg-odoo-success/15 text-odoo-success' },
  cancelled:  { label: 'Annulée',       cls: 'bg-odoo-danger/15 text-odoo-danger' },
};
    ...
    <div className="max-w-4xl mx-auto px-4 lg:px-6 py-6">
      ...
      {orders.length === 0 ? (
        <div className="text-center py-16 card">
          <Package className="w-10 h-10 text-odoo-muted mx-auto mb-3" />
        ...
        <div className="space-y-3">
          {orders.map((o) => (
            <button key={o.id} onClick={() => setView({ kind: 'order', id: o.id })}
              className="card w-full p-4 flex items-center justify-between hover:border-odoo-primary transition text-left">
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                <span className="font-bold text-odoo-primary">{formatPrice(o.total)}</span>
                <ChevronRight className="w-5 h-5 text-odoo-muted" />
              </div>
            </button>
          ))}
        </div>
    ...
    <div className="max-w-4xl mx-auto px-4 lg:px-6 py-6">
      ...
      <div className="card p-5 mb-4">
        <div className="flex items-start justify-between flex-wrap gap-2">
          <div>
            <p className="text-xs text-odoo-muted">Commande</p>
        ...
        <div className="grid sm:grid-cols-2 gap-3 mt-4 text-sm">
          <div className="flex items-start gap-2">
            <Calendar className="w-4 h-4 text-odoo-muted mt-0.5" />
            <div><p className="text-odoo-muted text-xs">Date</p><p>{formatDate(order.created_at)}</p></div>
          </div>
          <div className="flex items-start gap-2">
            <Phone className="w-4 h-4 text-odoo-muted mt-0.5" />
            <div><p className="text-odoo-muted text-xs">Contact</p><p>{order.customer_phone || '—'}</p></div>
          </div>
          <div className="flex items-start gap-2 sm:col-span-2">
            <MapPin className="w-4 h-4 text-odoo-muted mt-0.5" />
            <div><p className="text-odoo-muted text-xs">Adresse</p><p>{order.delivery_address || '—'}</p></div>
          </div>
        </div>
      </div>
      <div className="card mb-4">
        <h2 className="font-semibold p-4 border-b border-odoo-border">Articles</h2>
        <div className="divide-y divide-odoo-border">
          {items.map((it) => (
            <div key={it.id} className="p-4 flex justify-between gap-3">
              <div><p className="font-medium text-sm">{it.product_name}</p><p className="text-xs text-odoo-muted">{it.quantity} × {formatPrice(it.unit_price)}</p></div>
              <span className="font-semibold text-sm">{formatPrice(it.subtotal)}</span>
            </div>
          ))}
        </div>
        <div className="p-4 border-t border-odoo-border flex justify-between items-baseline bg-odoo-surface/50">
          <span className="font-semibold">Total</span>
          <span className="text-2xl font-bold text-odoo-primary">{formatPrice(order.total)}</span>
        </div>
      </div>
