...
const TYPE_META: Record<MovementType, { label: string; color: string; sign: '+' | '-'; icon: string }> = {
  sale_online: { label: 'Vente web',       color: 'text-odoo-primary bg-odoo-primary/10',   sign: '-', icon: '🛒' },
  sale_pos:    { label: 'Vente POS',        color: 'text-odoo-info bg-odoo-info/10',          sign: '-', icon: '🏪' },
  purchase:    { label: 'Achat / Entrée',   color: 'text-odoo-success bg-odoo-success/10',    sign: '+', icon: '📦' },
  stock_in:    { label: 'Entrée stock',     color: 'text-odoo-success bg-odoo-success/10',    sign: '+', icon: '⬆️' },
  damaged:     { label: 'Produit endommagé',color: 'text-odoo-danger bg-odoo-danger/10',      sign: '-', icon: '⚠️' },
  adjustment:  { label: 'Ajustement',       color: 'text-odoo-muted bg-odoo-surface',         sign: '+', icon: '✏️' },
};
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <PackageSearch className="w-6 h-6 text-odoo-primary" />Gestion des stocks
          </h1>
          <p className="text-sm text-odoo-muted mt-1">{products.length} produits actifs</p>
        </div>
        ...
        <>
          ...
          {(outOfStock.length > 0 || lowStockProducts.length > 0) && (
            <div className="grid sm:grid-cols-2 gap-3 mb-5">
              {outOfStock.length > 0 && (
                <div className="flex items-start gap-3 bg-odoo-danger/5 border border-odoo-danger/20 rounded-lg p-4">
                  <AlertTriangle className="w-5 h-5 text-odoo-danger flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="font-semibold text-sm text-odoo-danger">{outOfStock.length} produit{outOfStock.length > 1 ? 's' : ''} en rupture</p>
                    <p className="text-xs text-odoo-muted mt-0.5">{outOfStock.slice(0, 3).map((p) => p.name).join(', ')}{outOfStock.length > 3 ? '…' : ''}</p>
                  </div>
                </div>
              )}
              {lowStockProducts.length > 0 && (
                <div className="flex items-start gap-3 bg-odoo-warning/5 border border-odoo-warning/20 rounded-lg p-4">
                  <AlertTriangle className="w-5 h-5 text-odoo-warning flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="font-semibold text-sm text-odoo-warning">{lowStockProducts.length} produit{lowStockProducts.length > 1 ? 's' : ''} en stock faible</p>
                    <p className="text-xs text-odoo-muted mt-0.5">{lowStockProducts.slice(0, 3).map((p) => p.name).join(', ')}{lowStockProducts.length > 3 ? '…' : ''}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Tabs */}
          <div className="flex gap-1 border-b border-odoo-border mb-5">
            {([
              ...
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition ${tab === t.id ? 'border-odoo-primary text-odoo-primary' : 'border-transparent text-odoo-muted hover:text-odoo-dark'}`}>
                {t.icon}{t.label}
              </button>
            ))}
          </div>
          ...
          {tab === 'stock' && (
            <div className="card overflow-hidden">
              <div className="p-4 border-b border-odoo-border">
                <p className="text-sm text-odoo-muted">Niveaux actuels — mis à jour automatiquement après chaque vente ou mouvement.</p>
              </div>
              <div className="overflow-x-auto">
                    ...
                    {products.map((p) => {
                      ...
                      return (
                        <tr key={p.id} className={`hover:bg-odoo-surface/50 ${isOut ? 'bg-odoo-danger/3' : ''}`}>
                          ...
                          <td className="p-3 text-right hidden md:table-cell text-odoo-muted text-xs">{p.low_stock_threshold}</td>
                          <td className="p-3 text-center">
                            {isOut ? (
                              <span className="badge bg-odoo-danger/15 text-odoo-danger text-xs">Rupture</span>
                            ) : isLow ? (
                              <span className="badge bg-odoo-warning/15 text-odoo-warning text-xs">Faible</span>
                            ) : (
                              <span className="badge bg-odoo-success/15 text-odoo-success text-xs">OK</span>
                            )}
                          </td>
                          <td className="p-3 text-right hidden lg:table-cell text-odoo-muted text-xs">{formatPrice(p.price)}</td>
                ...
                {products.length === 0 && <p className="p-8 text-center text-odoo-muted">Aucun produit actif</p>}
              </div>
              {/* Stock value summary */}
              <div className="p-4 border-t border-odoo-border bg-odoo-surface flex flex-wrap gap-4 justify-end">
                <div className="text-sm">
                  <span className="text-odoo-muted">Valeur totale du stock : </span>
                  <span className="font-bold text-odoo-primary">{formatPrice(products.reduce((a, p) => a + p.price * p.stock, 0))}</span>
                </div>
              </div>
            </div>
          )}
            ...
            <>
              ...
              <div className="card overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-odoo-border">
                      {filteredMovements.map((m) => {
                        ...
                        return (
                          <tr key={m.id} className="hover:bg-odoo-surface/50">
                            <td className="p-3 text-odoo-muted text-xs whitespace-nowrap">{formatDate(m.created_at)}</td>
                            <td className="p-3 font-medium">{m.products?.name ?? '—'}</td>
                            <td className="p-3 text-center">
                              <span className={`badge text-xs ${meta.color}`}>{meta.label}</span>
                            </td>
                            <td className={`p-3 text-right font-bold ${isNeg ? 'text-odoo-danger' : 'text-odoo-success'}`}>
                              {isNeg ? '' : '+'}{m.quantity}
                            </td>
                            <td className="p-3 hidden md:table-cell text-odoo-muted text-xs">{m.reference || '—'}</td>
                            <td className="p-3 hidden lg:table-cell text-odoo-muted text-xs">{m.notes || '—'}</td>
                            <td className="p-3">
                              <button onClick={() => deleteMovement(m.id, m.product_id, m.quantity)}
                                className="text-odoo-muted hover:text-odoo-danger transition-colors" title="Supprimer et inverser">
                  ...
                  {filteredMovements.length === 0 && <p className="p-8 text-center text-odoo-muted">Aucun mouvement trouvé</p>}
                </div>
              </div>
            </>
          )}
          ...
          {tab === 'periods' && (
            <>
              <div className="card mb-4 p-4 bg-odoo-surface/60 border border-odoo-border">
                <p className="text-sm font-medium mb-1 flex items-center gap-2">
                  <ClipboardList className="w-4 h-4 text-odoo-primary" />Formule de calcul
                </p>
                <p className="text-sm text-odoo-muted font-mono bg-white border border-odoo-border rounded px-3 py-2 inline-block">
              ...
              {showPeriodForm && (
                <div className="card p-5 mb-5 border-l-4 border-l-odoo-primary">
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-3">
                    ...
                    {periodForm.opening_stock !== '' && (
                      <div className="flex flex-col justify-end">
                        <label className="block text-xs font-medium mb-1 text-odoo-success">= Stock Vendu (calculé)</label>
                        <div className="input bg-odoo-success/5 border-odoo-success/30 text-odoo-success font-bold text-sm flex items-center">
                          {Math.max(0,
                  ...
                  {periodError && <p className="text-xs text-odoo-danger mb-2">{periodError}</p>}
              ...
              <div className="card overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-odoo-surface text-xs font-medium text-odoo-muted uppercase">
                      <tr>
                        ...
                        <th className="p-3 text-right font-bold text-odoo-primary">= Vendus</th>
                        <th className="p-3 w-10" />
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-odoo-border">
                      {periods.map((p) => (
                        <tr key={p.id} className="hover:bg-odoo-surface/50">
                          <td className="p-3">
                            ...
                            <p className="text-xs text-odoo-muted">{p.start_date} → {p.end_date}</p>
                          </td>
                          <td className="p-3 text-odoo-muted">{p.products?.name ?? '—'}</td>
                          <td className="p-3 text-right">{p.opening_stock}</td>
                          <td className="p-3 text-right text-odoo-success">+{p.total_in}</td>
                          <td className="p-3 text-right text-odoo-danger">−{p.total_damaged}</td>
                          <td className="p-3 text-right">{p.closing_stock}</td>
                          <td className="p-3 text-right font-bold text-odoo-primary text-base">{p.sold_qty}</td>
                          <td className="p-3">
                            <button onClick={() => deletePeriod(p.id)} className="text-odoo-muted hover:text-odoo-danger transition-colors">
                  ...
                  {periods.length === 0 && (
                    <p className="p-8 text-center text-odoo-muted text-sm">Aucun inventaire périodique. Cliquez sur "Nouvel inventaire" pour commencer.</p>
                  )}
                </div>
              </div>
            </>
          ...
          {showMovForm && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md p-6">
                <button onClick={() => setShowMovForm(false)} className="absolute top-4 right-4 text-odoo-muted hover:text-odoo-dark"><X className="w-5 h-5" /></button>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-odoo-muted mt-1">
                ...
                {movError && <p className="text-xs text-odoo-danger mt-3">{movError}</p>}
