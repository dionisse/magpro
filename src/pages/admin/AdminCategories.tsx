  ...
  return (
    <div className="max-w-5xl mx-auto px-4 lg:px-6 py-6">
      <div className="flex items-center justify-between mb-4">
        <div><h1 className="text-2xl font-bold">Catégories</h1><p className="text-sm text-odoo-muted">{categories.length} catégories</p></div>
        <button onClick={() => setCreating(true)} className="btn-primary"><Plus className="w-4 h-4" />Nouvelle catégorie</button>
      </div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {categories.map((c) => (
          <div key={c.id} className="card overflow-hidden">
            <div className="aspect-video bg-odoo-surface">
              {c.image_url ? <img src={c.image_url} alt={c.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><Package2 className="w-10 h-10 text-odoo-muted" /></div>}
            </div>
            <div className="p-3">
              <p className="font-medium">{c.name}</p>
              <p className="text-xs text-odoo-muted line-clamp-2 mt-0.5 min-h-8">{c.description || '—'}</p>
              <div className="flex justify-end gap-1 mt-2">
                <button onClick={() => setEditing(c)} className="p-1.5 text-odoo-muted hover:text-odoo-primary hover:bg-odoo-primary/10 rounded transition"><Edit2 className="w-4 h-4" /></button>
                <button onClick={() => setDeletingId(c.id)} className="p-1.5 text-odoo-muted hover:text-odoo-danger hover:bg-odoo-danger/10 rounded transition"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          </div>
        ))}
