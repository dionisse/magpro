        ...
        {activeTab === 'base' && (
          <div className="space-y-4">
            ...
            <div>
              ...
              {optimizing && (
                <div className="mt-2 flex items-center gap-2 text-xs text-brand-primary bg-brand-primary/[0.08] border border-brand-primary/20 rounded-lg px-3 py-2">
                  <Loader2 className="w-3.5 h-3.5 animate-spin flex-shrink-0" />
                  Optimisation en cours — conversion WebP, compression, hébergement CDN…
                </div>
              )}
              {optimizeInfo && !optimizing && (
                <div className="mt-2 flex items-center gap-2 text-xs text-brand-success bg-brand-success/[0.08] border border-brand-success/20 rounded-lg px-3 py-2">
...
function BulkRow({ row, onChange, onRemove }: {
  ...
  return (
    <tr className={`group ${row._error ? 'bg-brand-danger/5' : row._dirty ? 'bg-brand-warning/[0.03]' : ''}`}>
