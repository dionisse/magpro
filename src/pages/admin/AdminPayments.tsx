...
const STATUS_META: Record<PaymentStatus, { label: string; color: string; icon: React.ReactNode }> = {
  pending:  { label: 'En attente',  color: 'bg-odoo-warning/15 text-odoo-warning',  icon: <Clock className="w-3.5 h-3.5" /> },
  paid:     { label: 'Payé',        color: 'bg-odoo-success/15 text-odoo-success',  icon: <CheckCircle className="w-3.5 h-3.5" /> },
  failed:   { label: 'Échoué',      color: 'bg-odoo-danger/15 text-odoo-danger',    icon: <XCircle className="w-3.5 h-3.5" /> },
  refunded: { label: 'Remboursé',   color: 'bg-odoo-muted/20 text-odoo-muted',      icon: <RotateCcw className="w-3.5 h-3.5" /> },
  partial:  { label: 'Partiel',     color: 'bg-odoo-info/15 text-odoo-info',        icon: <AlertTriangle className="w-3.5 h-3.5" /> },
};
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-6 py-6">
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <CreditCard className="w-6 h-6 text-odoo-primary" />Gestion des paiements
          </h1>
          <p className="text-sm text-odoo-muted mt-1">Suivi de tous les paiements manuels et en ligne</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <div className="card p-4 border-l-4 border-l-odoo-success">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-odoo-muted uppercase font-medium">Montant encaissé</p>
            <div className="bg-odoo-success/10 text-odoo-success rounded-md p-1.5"><DollarSign className="w-4 h-4" /></div>
          </div>
          <p className="text-xl font-bold text-odoo-success">{formatPrice(totalPaid)}</p>
          <p className="text-xs text-odoo-muted mt-1">{payments.filter((p) => p.status === 'paid').length} paiements</p>
        </div>
        <div className="card p-4 border-l-4 border-l-odoo-warning">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-odoo-muted uppercase font-medium">En attente</p>
            <div className="bg-odoo-warning/10 text-odoo-warning rounded-md p-1.5"><Clock className="w-4 h-4" /></div>
          </div>
          <p className="text-xl font-bold text-odoo-warning">{formatPrice(totalPending)}</p>
          <p className="text-xs text-odoo-muted mt-1">{payments.filter((p) => p.status === 'pending').length} paiements</p>
        </div>
        ...
        <div className="card p-4 border-l-4 border-l-odoo-danger">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-odoo-muted uppercase font-medium">Échoués</p>
            <div className="bg-odoo-danger/10 text-odoo-danger rounded-md p-1.5"><XCircle className="w-4 h-4" /></div>
          </div>
          <p className="text-xl font-bold text-odoo-danger">{formatPrice(totalFailed)}</p>
          <p className="text-xs text-odoo-muted mt-1">{payments.filter((p) => p.status === 'failed').length} paiements</p>
        </div>
      </div>
        ...
        <div className="card p-5 mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {Object.entries(byMethod).sort((a, b) => b[1] - a[1]).map(([method, amount]) => (
              <div key={method} className="bg-odoo-surface rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-odoo-muted">{METHOD_ICONS[method as PaymentMethod]}</span>
                  <p className="text-xs text-odoo-muted truncate">{METHOD_LABELS[method as PaymentMethod] ?? method}</p>
                </div>
                <p className="font-bold text-sm">{formatPrice(amount)}</p>
                <div className="mt-1.5 h-1 bg-odoo-border rounded-full overflow-hidden">
                  <div className="h-full bg-odoo-primary rounded-full" style={{ width: `${totalPaid > 0 ? (amount / totalPaid) * 100 : 0}%` }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-odoo-border">
                {filtered.map((p) => (
                  <>
                    <tr key={p.id} className={`hover:bg-odoo-surface/50 ${expandedId === p.id ? 'bg-odoo-surface/30' : ''}`}>
                      <td className="p-3">
                      ...
                      <td className="p-3 text-odoo-muted text-xs whitespace-nowrap">{formatDate(p.created_at)}</td>
                      <td className="p-3 font-mono text-xs font-medium text-odoo-primary">{p.order_number}</td>
                      <td className="p-3 hidden md:table-cell">
                        <div>
                          <p className="font-medium text-xs">{p.payer_name || '—'}</p>
                          <p className="text-odoo-muted text-xs">{p.payer_phone}</p>
                        </div>
                      </td>
                      <td className="p-3 text-center">
                        <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-odoo-surface text-odoo-muted text-xs">
                      ...
                      <td className="p-3 text-right font-bold text-odoo-primary">{formatPrice(Number(p.amount))}</td>
                      <td className="p-3 hidden lg:table-cell text-odoo-muted text-xs font-mono truncate max-w-32">
                        {p.transaction_id || '—'}
                      </td>
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          {p.chariow_checkout_url && (
                            <a href={p.chariow_checkout_url} target="_blank" rel="noopener noreferrer" title="Lien Chariow"
                              className="p-1.5 text-odoo-muted hover:text-odoo-info hover:bg-odoo-info/10 rounded-md transition">
                    ...
                    {expandedId === p.id && (
                      <tr key={p.id + '-detail'}>
                        <td colSpan={9} className="p-0 bg-odoo-surface/40">
                          <div className="px-6 py-4 border-t border-odoo-border grid sm:grid-cols-3 gap-4 text-sm">
                            <div>
                              <p className="text-xs text-odoo-muted uppercase mb-1">Mode de paiement</p>
                              <p className="font-medium flex items-center gap-1.5">{METHOD_ICONS[p.method]}{METHOD_LABELS[p.method]}</p>
                            </div>
                            <div>
                              <p className="text-xs text-odoo-muted uppercase mb-1">Référence transaction</p>
                              <p className="font-mono text-xs">{p.transaction_id || '—'}</p>
                            </div>
                            {p.chariow_checkout_url && (
                              <div>
                                <p className="text-xs text-odoo-muted uppercase mb-1">Lien Chariow</p>
                                <a href={p.chariow_checkout_url} target="_blank" rel="noopener noreferrer"
                                  className="text-odoo-info text-xs hover:underline flex items-center gap-1">
                            ...
                            {p.notes && (
                              <div className="sm:col-span-3">
                                <p className="text-xs text-odoo-muted uppercase mb-1">Notes</p>
                                <p>{p.notes}</p>
                              </div>
                            )}
                          </div>
            ...
            {filtered.length === 0 && (
              <p className="p-8 text-center text-odoo-muted text-sm">Aucun paiement trouvé</p>
            )}
          </div>
        </div>
      )}
      ...
      {editingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <button onClick={() => setEditingId(null)} className="absolute top-4 right-4 text-odoo-muted hover:text-odoo-dark">
              <X className="w-5 h-5" />
            </button>
                  ...
                  {(Object.keys(STATUS_META) as PaymentStatus[]).map((s) => (
                    <button key={s} type="button" onClick={() => setEditStatus(s)}
                      className={`flex items-center gap-2 p-2.5 border rounded-lg text-sm transition ${
                        editStatus === s ? 'border-odoo-primary bg-odoo-primary/5' : 'border-odoo-border hover:border-odoo-primary/50'
                      }`}>
                      <span className={STATUS_META[s].color.includes('success') ? 'text-odoo-success' : STATUS_META[s].color.includes('warning') ? 'text-odoo-warning' : STATUS_META[s].color.includes('danger') ? 'text-odoo-danger' : STATUS_META[s].color.includes('info') ? 'text-odoo-info' : 'text-odoo-muted'}>
                        {STATUS_META[s].icon}
                      </span>
