  ...
  if (items.length === 0) {
    return (
      <div className="max-w-3xl mx-auto px-4 lg:px-6 py-16 text-center">
        <div className="w-16 h-16 bg-odoo-surface rounded-full flex items-center justify-center mx-auto mb-4">
          <ShoppingBag className="w-8 h-8 text-odoo-muted" />
        </div>
        <h1 className="text-xl font-semibold mb-2">Votre panier est vide</h1>
        <p className="text-odoo-muted mb-6">Ajoutez des produits pour commencer.</p>
    ...
    <div className="max-w-5xl mx-auto px-4 lg:px-6 py-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-3">
          {items.map((item) => {
            ...
            return (
              <div key={key} className="card p-3 flex gap-3">
                <div className="w-20 h-20 bg-odoo-surface rounded flex-shrink-0 overflow-hidden">
                  {firstImage
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      {item.optionLabel && (
                        <span className="text-xs text-odoo-primary font-medium bg-odoo-primary/8 px-2 py-0.5 rounded-full mt-1 inline-block">
                          {item.optionLabel}
                        </span>
                      )}
                    </div>
                    <button onClick={() => removeFromCart(key)} className="text-odoo-muted hover:text-odoo-danger transition flex-shrink-0">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                  {bulkActive && <span className="badge bg-odoo-success/10 text-odoo-success mt-1"><Tag className="w-3 h-3 mr-1" />Prix de gros</span>}
                  <div className="mt-2 flex items-center justify-between">
                    <div className="flex items-center border border-odoo-border rounded">
                      <button onClick={() => updateQuantity(key, item.quantity - 1)} className="p-1.5 hover:bg-odoo-surface transition"><Minus className="w-3.5 h-3.5" /></button>
                      <span className="px-3 text-sm font-medium">{item.quantity}</span>
                      <button onClick={() => updateQuantity(key, item.quantity + 1)} disabled={item.quantity >= maxQty}
                        className="p-1.5 hover:bg-odoo-surface transition disabled:opacity-40 disabled:cursor-not-allowed"><Plus className="w-3.5 h-3.5" /></button>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-odoo-primary text-sm">{formatPrice(price * item.quantity)}</p>
                      <p className="text-xs text-odoo-muted">{formatPrice(price)}/unité</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <button onClick={clearCart} className="text-sm text-odoo-muted hover:text-odoo-danger transition flex items-center gap-1.5">
        ...
        <div className="lg:sticky lg:top-20 h-fit">
          <div className="card p-5">
            ...
            <div className="space-y-2 text-sm pb-4 border-b border-odoo-border">
              <div className="flex justify-between"><span className="text-odoo-muted">Sous-total</span><span className="font-medium">{formatPrice(subtotal)}</span></div>
              <div className="flex justify-between"><span className="text-odoo-muted">Livraison</span><span className="text-odoo-muted text-xs">Calculée à la commande</span></div>
            </div>
